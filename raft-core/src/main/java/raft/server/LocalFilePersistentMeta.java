...
public class LocalFilePersistentMeta implements RaftPersistentMeta {
    ...
    public LocalFilePersistentMeta(String stateFileDir, String raftId, boolean syncWriteFile) {
        ...
        stateFileDirPath = Paths.get(stateFileDir);
        ...
        stateFilePath = Paths.get(stateFileDir + File.separator + fileNamePrefix + "_" + raftId);
            ...
            if (Files.exists(stateFilePath)) {
                try {
                    if (Files.size(stateFilePath) > 512) {
                        throw new IllegalStateException(String.format("broken raft persistent file: \"%s\". it is too large",
                                stateFilePath));
                    }
    ...
    private void persistent() {
        ...
        logger.info("Save raft persistent meta success at path={} with term={}, votedFor={}, commitIndex={}",
                stateFilePath,
                term,
                votedFor,
                commitIndex);
